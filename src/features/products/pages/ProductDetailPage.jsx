...
export default function ProductDetailPage() {
  ...
  const { reviews, addReview, loading: reviewsLoading } = useReviews(product?.id);

  const isFavorite = product && favorites.some((f) => f.id === product.id);
  const rating = product ? (reviews.length ? reviews.reduce((a, r) => a + r.rating, 0) / reviews.length : product.rating ?? 0) : 0;
          ...
          <div className="mt-2 flex items-center gap-2">
            <div className="flex">
              {[1, 2, 3, 4, 5].map((s) => (
                <Star
                  key={s}
                  className={`h-5 w-5 ${s <= Math.round(displayRating) ? "fill-amber-400 text-amber-400" : "text-gray-200"}`}
                />
              ))}
            </div>
            <span className="text-sm text-emerald-600">({reviewCount} rəy)</span>
          </div>
        ...
        <div className="p-6">
          ...
          {activeTab === "reviews" && (
            <div className="space-y-4">
              ...
              {reviewsLoading ? (
                <p className="text-sm text-emerald-600">Yüklənir...</p>
              ) : (
