...
export default function FavoritesPage() {
  ...
  if (!items.length) {
    return (
      <div className="flex min-h-[60vh] flex-col items-center justify-center gap-4 p-6">
        ...
        <p className="text-lg text-emerald-700">Favoriləriniz boşdur</p>
        <Link to="/">
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-4">
      <h1 className="text-2xl font-bold text-emerald-900">Favorilər</h1>
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        {items.map((p) => (
          <Card key={p.id} className="overflow-hidden border-emerald-100 group">
            <Link to={"/product/" + p.id} state={{ product: p }} className="block">
              <div className="relative aspect-[4/3] overflow-hidden bg-emerald-50">
                {(p.images?.[0] || p.imageUrl) ? (
                  ...
                  <div className="flex h-full w-full items-center justify-center text-emerald-400">Şəkil yoxdur</div>
                )}
            ...
            <div className="p-3">
              <p className="font-medium text-emerald-900 line-clamp-2">{p.name || p.title}</p>
              <p className="text-sm text-emerald-600">{p.farmerName}</p>
              <div className="mt-2 flex items-center justify-between">
                <span className="font-semibold text-emerald-800">{(p.price ?? 0).toFixed(2)} ₼</span>
                <span className="text-xs text-emerald-500">★ {p.rating ?? 0}</span>
              </div>
              <Button className="mt-2 w-full" size="sm" onClick={() => dispatch(addToCart({ product: p }))}>
