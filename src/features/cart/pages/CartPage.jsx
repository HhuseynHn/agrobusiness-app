    ...
    <div className="mx-auto max-w-6xl space-y-6 p-4">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
            ...
            return (
              <Card key={item.productId} className="flex gap-4 p-4 border-emerald-100">
                <div className="h-24 w-24 shrink-0 overflow-hidden rounded-lg bg-emerald-50">
                  {imgSrc ? (
                    <LazyImage src={imgSrc} alt={p?.name || p?.title} className="h-full w-full object-cover" />
                  ) : (
                    <div className="flex h-full w-full items-center justify-center text-xs text-emerald-400">Şəkil</div>
                  )}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-emerald-900">{p?.name || p?.title}</p>
                  <p className="text-sm text-emerald-600">{p?.farmerName}</p>
                  <p className="mt-1 font-semibold text-emerald-800">{(p?.price ?? 0).toFixed(2)} ₼</p>
                  <div className="mt-2 flex items-center gap-2">
                    <button type="button" onClick={() => dispatch(setQuantity({ productId: p.id, quantity: qty - 1 }))} className="rounded border border-emerald-200 p-1 text-emerald-700 hover:bg-emerald-50">
                ...
                <div className="flex flex-col items-end justify-between">
                  ...
                  <div className="flex gap-1">
                    <Button variant="ghost" size="sm" onClick={() => dispatch(addFavorite(p))}>Favorilərə</Button>
                    <button type="button" onClick={() => dispatch(removeFromCart(p.id))} className="rounded p-2 text-red-600 hover:bg-red-50" aria-label="Sil">
        ...
        <div className="lg:col-span-1">
          <Card className="sticky top-24 border-emerald-100 p-6">
              ...
              <div className="space-y-2">
                ...
                {DELIVERY_OPTIONS.map((opt) => (
                  <label key={opt.id} className="flex cursor-pointer items-center gap-2">
                    <input type="radio" name="delivery" checked={delivery === opt.id} onChange={() => setDelivery(opt.id)} className="text-emerald-600" />
                    <span>{opt.label}</span>
